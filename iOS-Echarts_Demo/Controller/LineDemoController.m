...
+ (void)load
{
    if (!isCatchAction) {
        ...
        [NSObject swizzleMethod:selfClass srcSel:@selector(respondsToSelector:) tarClass:selfClass tarSel:@selector(respondsToSelector_debug:)];
    }
}

- (BOOL)respondsToSelector_debug:(SEL)aSelector
{
    DLog(@"%@",NSStringFromSelector(aSelector));
    return [self respondsToSelector_debug:aSelector];
}
...
- (IBAction)kDemosClick:(id)sender {
    ...
    switch (btn.tag) {
        ...
        case LindDemoTypeBtnTagStackedArea:
            ...
            break;
        case LindDemoTypeBtnTagIrregularLine://10005
...
-(void)showLineDemo {
    ...
    option.legend = [[PYLegend alloc] init];
    option.legend.x  = @(_kEchartView.frame.size.width - 200);
    ...
    series2.data = @[@(120), @(82), @{@"value":@(201), @"symbol":@"start", @"symbolSize":@(15),@"itemStyle":@{@"normal":@{@"label":@{@"show":@(YES),@"textStyle":@{@"fontSize":@(10),@"fontFamily":@"微软雅黑",@"fontWeight":@"bold"}}}}}, @{@"value":@(134),@"symbol":@"none"}, @(190), @{@"value":@(230),@"symbol":@"emptypin",@"symbolSize":@(8)}, @(10)];
    [serieses addObject:series2];
    ...
    series5.itemStyle.normal.lineStyle.color = PYRGBA(220, 20, 60, 1);//@"(function (){var zrColor = require('zrender/tool/color');return zrColor.getLinearGradient(0, 0, 1000, 0,[[0, 'rgba(255,0,0,0.8)'],[0.8, 'rgba(255,255,0,0.8)']])})()";
